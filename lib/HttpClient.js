...
function HttpClient(option) {
    ...
    var list = hosts.split(','); // 传进来的字符串用逗号分隔多个地址
    /**
     * 配置服务器的 HOST 列表
     * @type {Array.<string>}
     * @private
...
HttpClient.prototype._doRequest = function (hostIndex, retryTimes, path, cb) {
    ...
    var _retryTimes = self._retryTimes;
    var _retrySleepSeconds = self._retrySleepSeconds;
    var _doRequest = self._doRequest;
    request(url, function (err, response, body) {
        if (err || response.statusCode != 200) {
            log.warn('One request failed! url: %s, err/res: %s', url, err ? err.stack : response.statusCode);
            setTimeout(function () {
                if (retryTimes < _retryTimes) {
                    _doRequest(hostIndex, retryTimes + 1, path, cb);
                } else {
                    _doRequest(hostIndex + 1, 0, path, cb);
                }
            }, _retrySleepSeconds * 1000);
        } else {
            // 成功则解析并回调数据
            try {
                if (_.isString(body)) {
...
HttpClient.prototype.getZkInfo = function (cb) {
    var self = this;
    var _request = self._request;
    _request(ZOO_PATH.HOSTS, function (hostsErr, body) {
        if (hostsErr) {
            return cb(hostsErr);
        }
        var zooHosts = body.value;
        _request(ZOO_PATH.PREFIX, function (prefixErr, body) {
            if (prefixErr) {
                return cb(prefixErr);
            }

...
var warn = debug('node-disconf-client:wran');
...
DisconfClient.prototype.init = function (file, option, cb) {
    ...
    fs.access(_path, fs.F_OK, function (err) {
        if(!err) {
            ...
            warn('The DisconfClient\'s option file is not exist. err: ', err.stack);
            self.getZkInfo(cb);
        }
    });
};
...
DisconfClient.prototype.getZkInfo = function (cb) {
    var self = this;
    var httpClient = self._httpClient = new HttpClient(self._option);
    httpClient.getZkInfo(function (err, host, prefix) {
        if(err){
            cb(err);
        }else{
            try{
                var zk = zookeeper.createClient(host, {
                    sessionTimeout: ZK_SESSION_TIMEOUT,
                    spinDelay : ZK_SPIN_DELAY,
                    retries : ZK_RETRIES
                });
                zk.connect();
                self._zkClient = new ZkClient(zk, prefix, self._option);
                self.buildConfigNodes();
                cb(null, zk);
            }catch (err){
                cb(err);
            }
 ...
 * 维护配置文件和配置项的列表
 */
DisconfClient.prototype.buildConfigNodes = function () {
    var self = this;
    var types = ['file', 'item'];
    var nodeList = [];
    var readyNum = 0;
    types.forEach(function (type) {
        ...
        list.forEach(function (name) {
            if (name == '') return;
            nodeList.push(new ConfigNode(type, name, self._httpClient, self._zkClient));
        });
        nodeList.forEach(function (node) {
            node.init(function (event) {
                self.emit('change', event, node);
            },function (err, data) {

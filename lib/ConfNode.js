...
ConfigNode.prototype._doWatch = function (path, event, watcher, callback) {
    var self = this;
    var _doWatch = self._doWatch;
    var _getData = self._getData;
    self._zk.getData(path,
        function (event) {
            _doWatch(path, event, watcher, callback);
        },
...
ConfigNode.prototype._getData = function (callback) {
    ...
    self._http.getConfig(self._type, self._key, function (err, data) {
        ...
        try {
            ...
            write(
                sysPath.join(_option.user_define_download_dir, _key),
                data,
                function (err) {
                    if (err) {
                        log.warn('save the configuration data to a file error. key: %s, error: %s', _key, err.stack);
                    } else {
                        log.info('save the configuration data to a file success. key: %s', _key);
                    }
                }
            );
            ...
            if (_type === 'file') {
                ...
                for(var item in data){
                    if(data.hasOwnProperty(item) && data[item] === null){
                        log.warn('item [%s] of config file [%s] is null, now transform to empty string.', item, _key);
                        data[item] = '';
                    }
                }
                _pushData(JSON.stringify(data));
            } else {
                // item
                _pushData(data);
                data = properties.parse(_key + '=' + data);
            }
        } catch (e) {
            return callback(e);
        }
...
ConfigNode.prototype._pushData = function (data) {
    ...
    var _zk = self._zk;
    var _createEphemeralNode = self._createEphemeralNode;
    var _tempPath = self._tempPath;
    _zk.exists(_path, function (err, stat) {
        if (err) {
        ...
        if (stat) {
            ...
            // 临时节点还未创建则创建
            _createEphemeralNode(_tempPath, data, ephemeralHandle);
        }
    });
    return this;
};
...
ConfigNode.prototype._createPersistentNode = function (path, data, callback) {
    var self = this;
    var _zk = self._zk;
    _zk.exists(path, function (err, stat) {
        if (!err && !stat) {
